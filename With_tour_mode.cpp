...
int main() {
    ...
    while (running_program) {
        ...
        if (stop_btn && !last_stop && t.running) {
            ...
            t.running = false;
            t.paused = false;
            // Влизане в режим на обиколки след STOP
            if (!t.laps.empty()) {
                lap_mode = true;
                lap_display_index = 0;
                cout << "Автоматичен вход в режим на обиколки след STOP\n";
            } else {
                lap_mode = false;
            }
        ...
        if (lap_btn && !last_lap) {
            ...
            long delta = duration_cast<milliseconds>(now - last_lap_press_time).count();

            if (lap_mode) {
                if (delta < 500) {
                    // 🆕 Двойно натискане – връщане или изход
                    if (lap_display_index >= t.laps.size() - 1) {
                        lap_mode = false;
                        lap_display_index = 0;
                        cout << "Изход от режим обиколки\n";
                    } else {
                        lap_display_index = 0;
                        cout << "Връщане в началото на обиколките\n";
                    }
                } else {
                    // 🔁 Следваща обиколка
                    lap_display_index = (lap_display_index + 1) % t.laps.size();
                    cout << "Следваща обиколка: " << (lap_display_index + 1) << "\n";
                }
            } else if (t.running) {
    // Запис на обиколка
